...
const seed = async (data) => {
  const { endpointData, categoryData, commentData, reviewData, userData } =
    data;
  await db.query(`DROP TABLE IF EXISTS endpoints;`);
  await db.query(`DROP TABLE IF EXISTS comments;`);
  await db.query(`DROP TABLE IF EXISTS reviews;`);
  await db.query(`DROP TABLE IF EXISTS users;`);
  await db.query(`DROP TABLE IF EXISTS categories;`);

  const endpointsTablePromise = db.query(`
  ...
  const insertEndpointsQueryStr = format(
    "INSERT INTO endpoints (Endpoint, Method, Description) VALUES %L RETURNING *;",
    endpointData.map(({ Endpoint, Method, Description }) => [
      Endpoint,
      Method,
      Description,
    ])
  ...
  const endpointsPromise = db
    .query(insertEndpointsQueryStr)
  ...
  await Promise.all([endpointsPromise, categoriesPromise, usersPromise]);
